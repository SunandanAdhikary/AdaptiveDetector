...
K_new = {};
i = 1;
Knew = [0.0267    0.0027;0.0011    0.0034;0.0013    0.0030;...
        0.0018    0.0003;-0.0147   -0.0221;0.0054   -0.0640;...
        -0.0098   -0.0579;-0.0160   -0.0547;-0.0279   -0.0583;...
         -0.0291   -0.0513;-0.0284   -0.0438]
while solved1
    K_new{1} = Knew(i,:);%sdpvar(size(B,2),size(A,2));
    x(:,1) = x0;
    xhat(:,1) = xhat0;
    u(:,1) = -K_new{1}*xhat0;
    y(:,1) = C*x(:,1);
...
while ~inside
    if i > size(Knew,1)
        ...
        if solved == 0
            ...
            fprintf("can not find a suitable controller from " +value(x(:,i))+" to "+value(x(:,i+1))+"\n");
            break;
        end
        else
            inside = min((value(x(:,i+1)) >= [perf(1,:)']).*(value(x(:,i+1)) <= [perf(2,:)']))
