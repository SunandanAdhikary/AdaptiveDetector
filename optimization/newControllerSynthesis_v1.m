...
while solved ~= 0
    ...
    K_new = sdpvar(size(B,2),size(A,2));
...
%     for j = 1: size(A,1)
    %     constraints = [constraints,safex(:,1).' >= max(xa(:,i+1))]; 
        sol = optimize([constraints,x(:,n) >= perf(1,:)'],[],ops);
%         sol = optimize([constraints,esterr_safety_lim+0.001 >= norm((ea(j,i+1)-e(j,i+1)),1)-norm((eap(j,i+1)-ep(j,i+1)),1)]);
