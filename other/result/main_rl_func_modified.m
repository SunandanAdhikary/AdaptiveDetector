...
%% trajectory tracking
    ...
    s.init = 0.1;
    s.perf = 0.3
    s.th = 25.31;              % new: changed it a bit so that without attack, the residue remains below threshold.
    s.sensorRange = [30000];
    s.actuatorRange = [36];
    s.settlingTime = 13; 
end
...
s.avgfar = chi2cdf(s.th,1*size(s.C,1),'upper'); 
s.avgtpr = ncx2cdf(s.th,1*size(s.C,1),s.non_cent(1),'upper');
