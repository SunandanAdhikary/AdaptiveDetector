    ...
    while isSat == 0:  
        ...
        while (index<patternLen) and (isSat == 0):
            ...
            for i in range(K+1):
                ...
                for varcount in range(1,y_count+1):
                    ...
                    for varcount2 in range(1,x_count+1):
                        decl+="RObs"+str(varcount2)+str(varcount)+"_"+str(i+1)+" = Real('RObs"+str(varcount2)+str(varcount)+"_"+str(i+1)+"')\n"#new
                ...
                for varcount in range(1,x_count+1):
                    ...
                    for varcount2 in range(1,u_count+1):
                        decl+="RGain"+str(varcount2)+str(varcount)+"_"+str(i+1)+" = Real('RGain"+str(varcount2)+str(varcount)+"_"+str(i+1)+"')\n"#new
                for varcount in range(1,u_count+1):
            ...
            for i in range(K):
                ...
                # # Compute 1-norm of r
                # expr_rabs = "#Residue check by Detector\n"
                ...
                for varcount1 in range(1,x_count+1):
                    ...
                    for varcount4 in range(1,y_count+1):
                        expr_z+=" (("+str(L[varcount1-1,varcount4-1])+"+RObs"+str(varcount1)+str(varcount4)+"_"+str(i+1)+")*r"+str(varcount4)+"_"+str(i)+") +" #new
                ...
                if dropPattern[i]: #If there is a 1 in the pattern
                    ...
                    for varcount1 in range(1,u_count+1):
                        ...
                        for varcount2 in range(1,x_count+1):
                            if isDelayed:
                                expr_u+=" - (("+str(Gain[varcount1-1,varcount2-1])+"+RGain"+str(varcount1)+str(varcount2)+"_"+str(i+1)+")*z"+str(varcount2)+"_"+str(i)+")"#new
                            else:
                                expr_u+=" - (("+str(Gain[varcount1-1,varcount2-1])+"+RGain"+str(varcount1)+str(varcount2)+"_"+str(i+1)+")*z"+str(varcount2)+"_"+str(i+1)+")"#new
                        expr_u +=")\n"
                    f.write(expr_u)
                    ...
                    if i == (j+index):      # If in this iteration we can give an attack      
                        ...
                        for varcount1 in range(1,u_count+1):
                            ...
                            forallvars+="attackOnU"+str(varcount1)+"_"+str(i)+"," #new
                            # print(forallvars)
                        f.write(expr_uatk)
                        ...
                        if yAffected:
                            ...
                            for varcount1 in range(1,y_count+1):
                                ...
                                forallvars+="attackOnY"+str(varcount1)+"_"+str(i)+"," #new
                                # print(forallvars)
                                for varcount2 in range(1,x_count+1):
                ...
                f.write(expr_u)
                f.write(expr_uatk)#new
                # Update y when drop has no effect on y
                if yAffected==0:
                    if i == (j+index):
                        expr_y="#Sensor data/Output calc\n"
                        for varcount1 in range(1,y_count+1):
                            f.write("attackOnY"+str(varcount1)+"_"+str(i)+" = Real('attackOnY"+str(varcount1)+"_"+str(i)+"')\n")
                            expr_y+="s.add(y"+str(varcount1)+"_"+str(i+1)+" == ("+str(y_attack_map[varcount1-1])+"*attackOnY"+str(varcount1)+"_"+str(i)+")"
                            forallvars+="attackOnY"+str(varcount1)+"_"+str(i)+"," #new
                ...
                for varcount1 in range(1,y_count+1):
                    if y_attack_map[varcount1-1] and sensorRange[varcount1-1]!=0:
                        ...
                        expr_yatk+="s.add(And(attackOnY"+str(varcount1)+"_"+str(i)+">-"+str(sensorRange[varcount1-1])+",attackOnY"+str(varcount1)+"_"+str(i)+"<"+str(sensorRange[varcount1-1])+"))\n"#new
                f.write(expr_y)
                # f.write("\n")
                f.write(expr_yatk)#new
                f.write("\n")
            
            assertion="#Check state inclusion in next region: (1-(i+1)*(1-innerCircleDepth)/K)*safex\n"
            forallvars=forallvars[:len(forallvars)-1]
            assertion+="s.add(ForAll(["+forallvars+"],And(" #new
            # assertion+="s.add(And("
            for varcount in range(1,x_count+1):
                for i in range(K):
                    assertion+="xabs"+str(varcount)+"_"+str(i+1)+" < "+str((1-(i+1)*(1-innerCircleDepth)/K)*safex[varcount-1])+","#new
            assertion = assertion[:len(assertion)-1] #new
            f.write(assertion)
            f.write(")))\n")
        ...
        # depths [attackLen-1]=depths [attackLen]
    # final = open(path+modelName+"_rcontroller.result", "a+")
