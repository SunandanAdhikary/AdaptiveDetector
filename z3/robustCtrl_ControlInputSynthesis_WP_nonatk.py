# ultered : Control signal is neither computed nor communicated during drop
...
for pattern in set(patternList):
    ...
    toWrite= ""
    while (isSat == 1) and (todepth > 0):  
        index = start
        # while (index<patternLen) and (isSat == 0):
        # while (todepth > 0):
        # create drop pattern
        K = index + attackLen + offset
        print("Trying for robust controller in "+str(K)+" steps, from "+str(fromdepth)+"*safex to "+str(todepth))
        forallvars="" #new
        antecedent=""
        consequent=""
        ...
        fileName = modelName + "_robustCtrl_"+str(fromdepth)+"to"+ str(todepth)+"_1step_nonatk.py"
        f = open(path+fileName, "w+")
        headers= ""
        # f.write("from z3 import *\n")
        headers= headers + "from z3 import *\n"
        # f.write("import math\n")
        headers= headers + "import math\n"
        # f.write("import numpy as np\n\n")
        headers= headers + "import numpy as np\n\n"
        # f.write("s = Solver()\n")
        headers= headers + "s = Solver()\n"
        # f.write("set_option(precision=40)\n")
        # headers= headers + "set_option(precision=40)\n"
        # f.write("set_option(rational_to_decimal=True)\n")
        headers= headers + "set_option(rational_to_decimal=True)\n"
        ...
        for varcount in range(1,x_count+1):
            ...
            pydecl+= "xabs"+str(varcount)+" = np.zeros("+str(K+1)+", dtype=float)\n"

            # f.write("zabs"+str(varcount)+" = np.zeros("+str(K+1)+", dtype=float)\n")
            pydecl+= "zabs"+str(varcount)+" = np.zeros("+str(K+1)+", dtype=float)\n"

            # f.write("x"+str(varcount)+" = np.zeros("+str(K+1)+", dtype=float)\n")
            pydecl+= "x"+str(varcount)+" = np.zeros("+str(K+1)+", dtype=float)\n"
            # f.write("z"+str(varcount)+" = np.zeros("+str(K+1)+", dtype=float)\n")
            pydecl+= "z"+str(varcount)+" = np.zeros("+str(K+1)+", dtype=float)\n"
        for varcount in range(1,y_count+1):
            # f.write("y"+str(varcount)+" = np.zeros("+str(K+1)+", dtype=float)\n")
            pydecl+= "y"+str(varcount)+" = np.zeros("+str(K+1)+", dtype=float)\n"
            # f.write("attackOnY"+str(varcount)+" = np.zeros("+str(K+1)+", dtype=float)\n")
            # pydecl+= "attackOnY"+str(varcount)+" = np.zeros("+str(K+1)+", dtype=float)\n"
            # f.write("r"+str(varcount)+" = np.zeros("+str(K+1)+", dtype=float)\n")
            pydecl+= "r"+str(varcount)+" = np.zeros("+str(K+1)+", dtype=float)\n"
        for varcount in range(1,u_count+1):
            ...
            # pydecl += "uattacked"+str(varcount)+" = np.zeros("+str(K+1)+", dtype=float)\n"
            # f.write("attackOnU"+str(varcount)+" = np.zeros("+str(K+1)+", dtype=float)\n")
            # pydecl+= "attackOnU"+str(varcount)+" = np.zeros("+str(K+1)+", dtype=float)\n"
            
        # f.write("r = np.zeros("+str(K+1)+", dtype=float)\n\n")
        pydecl+= "r = np.zeros("+str(K+1)+", dtype=float)\n\n"
        # f.write("RGain={}\n") #new
        pydecl+= "RGain={}\n"
        # f.write("RObs={}\n") #new
        for ii in range(K+1):
            # f.write("RGain["+str(ii+1)+"] = np.zeros(("+str(u_count)+","+str(x_count)+"), dtype=float)\n")# new block
            pydecl+= "RGain["+str(ii+1)+"] = np.zeros(("+str(u_count)+","+str(x_count)+"), dtype=float)\n"
        ...
        for i in range(K+1):
            ...
            for varcount in range(1,y_count+1):
                ...
                # decl+="attackOnY"+str(varcount)+"_"+str(i)+" = Real('attackOnY"+str(varcount)+"_"+str(i)+"')\n"
            ...
            for varcount in range(1,u_count+1):
                ...
                # decl+="attackOnU"+str(varcount)+"_"+str(i)+" = Real('attackOnU"+str(varcount)+"_"+str(i)+"')\n"
        ...
        toWrite+= "\n#Init\n"
        for varcount in range(1,x_count+1):
            antecedent+="Or(x"+str(varcount)+"_0 == "+str(fromdepth)+"*"+str(safex[varcount-1])+",x"+str(varcount)+"_0 == "+str(fromdepth)+"*"+str(-safex[varcount-1])+"),\n"
            forallvars+="x"+str(varcount)+"_0,"
            antecedent+="And(z"+str(varcount)+"_0 <= "+str(fromdepth)+"*"+str(safex[varcount-1])+",z"+str(varcount)+"_0 >= "+str(fromdepth)+"*"+str(-safex[varcount-1])+"),\n"
        ...
        for i in range(K):
            ...
            for varcount1 in range(1,y_count+1):
                ...
                consequent+="y"+str(varcount1)+"_"+str(i)+" =="
                ...
                consequent= consequent[:len(consequent)-1]
                consequent+="\n"
        ...
        # todepth= (1-(i+1)*(1-innerCircleDepth)/K)
        
        for varcount in range(1,x_count+1):
            for i in range(K):
                consequent+="xabs"+str(varcount)+"_"+str(i+1)+" < "+str(todepth*safex[varcount-1])+",\n"
                consequent+="zabs"+str(varcount)+"_"+str(i+1)+" < "+str(todepth*safex[varcount-1])+",\n"
        # print(antecedent)
        # print("\n..and..\n")
        # print(consequent)
        #format consequent
        ...
        # print(noOfStatements)
        # print("consequent in original")
        # print(consequent+"\n")
        while noOfStatements>(x_count*K*2):
        ...
        execs+= "\nfsmt = open(\""+path+fileName+".smt2\", \"w+\")\n"
        # fsmt = open(path+modelName+"_"+str(K)+".smt2", w+)
        # f.write("fsmt.write(s.sexpr())\n")
        execs+= "fsmt.write(s.sexpr())\n"
        # f.write("fsmt.write(\"(check-sat)\")\n")
        execs+= "fsmt.write(\"(check-sat)\")\n"
        # f.write("fsmt.write(\"(get-model)\")\n")
        # execs+= "fsmt.write(\"(get-model)\")\n"
        # f.write("fsmt.close()\n")
        execs+= "fsmt.close()\n"
        # f.write("\nif s.check() != sat:\n")
        execs+= "\nif s.check() != sat:\n"
        ...
        execs+= "\tf0 = open(\""+path+modelName+"_robustC.z3result\", \"w+\")\n"
        # f.write("\tf0.write(\"1\")\n")
        execs+= "\tf0.write(\"1\")\n"
        # f.write("\tf0.close()\n") 
        execs+= "\tf0.close()\n"
        ...
        execs+= "\tfor d in m.decls():\n"
        # f.write("\t\tprint (\"%s = %s\" % (d.name(), m[d]))\n")
        execs+= "\t\tprint (\"%s = %s\" % (d.name(), m[d]))\n"
        ...
        if isSat==1:
            ...
            # os.system("rm -rf "+path+fileName)
            # attackLen = attackLen + 1#new
            if todepth != 0.995 :
                todepth+= 0.05
                fileName = modelName + "_robustCtrl_"+str(fromdepth)+"to"+ str(todepth)+"_1step.py"
                print("go to "+path+fileName+".z3out \n")
                # fromdepth = todepth
                # ans= input("next iteration from current depth "+str(todepth)+" to next to depth?[y/n]")
                # if ans == "y":
                #     isSat= 1
                #     todepth-= 0.05
