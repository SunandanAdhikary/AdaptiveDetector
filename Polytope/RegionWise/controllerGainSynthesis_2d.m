...
x0 = [-25; -30];
xhat0=[-25 + sqrt(threshold*nonatk_cov); -30+sqrt(threshold*nonatk_cov)];
...
obj = norm(xi-ub_init,Inf) + norm(xhati-ub_init_hat,Inf);

optimize(constraints, ...
    ub_init - abs(xi) + ub_init_hat - abs(xhati)...
        ,options)
check(constraints)
...
x_init = [-25, -30];
xhat_init=[-25 + (-1)*sqrt(threshold*nonatk_cov), -30+(-1)*sqrt(threshold*nonatk_cov)];
