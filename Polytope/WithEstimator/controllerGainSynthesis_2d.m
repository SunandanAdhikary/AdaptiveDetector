...
K1{end}=zeros(size(K));
L1 = sdpvar(repmat(size(L,1),t+1),repmat(size(L,2),t+1));
L1{end}=zeros(size(L));
...
for i=1:t    
    ...
    AA_i= [A zeros(size(A));(L + L1{i})*C (A-(L + L1{i})*C)] ;
    AA_cl_i= AA_i+BB*KK;
    constraints = [constraints, AA_cl_i'*P_lyap*AA_cl_i-P_lyap <= ...
        -0.001*eye(size(P_lyap)), -P_lyap <= -0.001*eye(size(P_lyap))];

    xi= A*xi+ B*ui;
    xhati= (L + L1{i})*C*xi+ (A-(L + L1{i})*C)*xhati + B*ui;
...
optimize([constraints, uncertain(z), -1 <= z <=1],...
    -scale,sdpsettings('debug',1,'bmibnb.maxiter',200,...
    'bmibnb.maxtime',7200))
check(constraints)
...
for i=1:time
    ...
    x = A*x + B*u;    
    if i<=t
        xhat = (L + L2{i})*C*x+(A-(L + L2{i})*C)*xhat+B*u;
    else
